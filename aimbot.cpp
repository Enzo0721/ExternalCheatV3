...
void aimbot::aimbotThread(const Memory& mem) noexcept
{
    while (gui::exit)
    {
        if (settings::toggle_aimbot)
        {
            ...
            const auto& aimPunch = mem.read<Vector3>(localBase + offsets::m_aimPunchAngle) * 2 * settings::aimbotRecoilAssist;

            auto bestFov = static_cast<float>(settings::aimbotFOV);
            ...
            for (int i = 1; i <= 32; i++)
            {
                ...
                /* Add this later */
                // FIX 3: Skip players who are NOT spotted by you. The '!' inverts the logic.
                ...
                /* Loop through all the bones that are selected */
                for (int bone : settings::aimbotBone)
                {
            ...
            if (!bestAngle.isZero())
                mem.write<Vector3>(clientState + offsets::dwClientState_ViewAngles, viewAngles + bestAngle / float(settings::aimbotSmoothness));
        }
    }
}